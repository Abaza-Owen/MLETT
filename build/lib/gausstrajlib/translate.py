...
class GaussianTranslator(): #Object that facillitates the reading of .log files and writing to .xyz files in proper format. Main class of module.
        ...
        def __write_to_xyz__(self, outname: str, Data: TrajData(), grad = True, format = 'sgdml', eng_cnst = True): #function internal to class for writing xyz format for sGDML
                try:
                    ...
                    with open(outname.split('.')[0]+ ".xyz", 'w') as xyz:
                        for j in range(num_xyz_sets):
                            index = int(math.floor(j/num_atoms))
                            if(s_e[index] > -0.058166 or not eng_cnst): #Energy cutoff for reactants in this range
                                if(j%num_atoms == 0):
        ...
        def __read_from_log_file__(self, filename: str):
                ...
                self.trainData.clear()  # resets
                try:
                    with open(filename, 'r') as log:
                        #create empty temporary lists and important values
                        sym_temp = []
                        xyz_temp = []
                        forces_temp = []
                        eng_pot_temp = []
                        ...
                        # Ensure data lengths match expected counts
        ...
        def write(self, filename, test = False, grad = True, format = 'sgdml', eng_cnst = True): #public function which calls internal __write_to_xyz_. Included for useability and to allow eventual expansion to other file types if necessary
                self.__write_to_xyz__(filename, self.trainData, grad = grad, format = format, eng_cnst = eng_cnst)
