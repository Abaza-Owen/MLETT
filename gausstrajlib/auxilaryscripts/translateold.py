...
class GaussianTranslator(): #Object that facillitates the reading of .log files and writing to .xyz files in proper format. Main class of module.
        ...
        def __write_to_xyz__(self, outname: str, Data: TrajData(), grad = True, format = 'sgdml', low_e = float('-inf'), high_e = float('inf')): #function internal to class for writing xyz format for sGDML
        ...
        def write(self, filename, test = False, grad = True, format = 'sgdml', low_e = float('-inf'), high_e = float('inf')): #public function which calls internal __write_to_xyz_. Included for useability and to allow eventual expansion to other file types if necessary
                self.__write_to_xyz__(filename, self.trainData, grad = grad, format = format, low_e = low_e, high_e = high_e )
