...
at every step of a Gaussian BOMD trajectory simulation. These extracted values are 
converted from atomic units (a.u) to Ang and kcal/mol, which is the standard
for sGDML. This is output as an extended xyz file (.xyz). This xyz file can be output in
formats accepted by either sGDML or MSA depedning on the command line options chosen.

This file can then be read directly from a python script using the sGDML.io.readxyz() function
...
If you have questions about this navigate to github and click on the down arrow near Code on the webpage and it will show you have to achive this.
____________________________________________________________________________________________________________________________________________________________________________
Usage:
...
option		function
-f		Parse and Write from one specified .log file (default)
-d 		Parse and Write from all .log files in -wd
-g		Include gradient/force in output XYZ, necessary for sGDML, optional for MSA
-nc		Don't filter out points wiht potential energies above a certain threshold 
-m		MSA file format (Force: Hartree/Bohr, Dist: Ang, Eng: Hartree)
-s 		SGDML file format (Force: kcal/mol*ang, Dist: Ang, Eng: kcal/mol) (default)
...
Otherwise, please email me a copy of the script that you are using the output file
it is not working correctly with, and I will try to modify the script to work as
soon as possible.
...
or Gaussview, where the input gjf was generated, but I am not sure which value is correct or which forces coerrespond correctly 
with the first step in these cases. If all other points match the log file in such a job,
